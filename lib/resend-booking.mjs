 ...
 * Customer confirmation text uses 07403 767600 — keep in line with `PHONE_DISPLAY` in `src/siteConfig.ts` if you change the number.
 */
 ...
 * @param {Record<string, string>} data
 */
export function buildCustomerConfirmationMessage(data) {
  const firstName = data.name.split(/\s+/)[0] || data.name;
  const subject = "We got your request — Dripline Plumbers";
  const lines = [
    `Hi ${data.name},`,
    "",
    "Thanks for getting in touch. Here’s a copy of what you sent to us. We will confirm with you by phone or email as soon as we can.",
    "",
    "--------",
    `Name:     ${data.name}`,
    `Phone:    ${data.phone}`,
    ...
    "",
    "--------",
    `If you need to reach us right away, call 07403 767600 (UK).`,
  ];
  const text = lines.join("\n");
  ...
  const html = `<!doctype html>
<html><body style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;font-size:15px;line-height:1.5;color:#0f172a;">
<p style="margin:0 0 8px;">Hi ${escapeHtml(firstName)},</p>
<p style="margin:0 0 20px;color:#334155;">Thanks for getting in touch. Below is a copy of your booking request. We&rsquo;ll be in touch by phone or email as soon as we can.</p>
<p style="margin:0 0 8px;font-size:12px;letter-spacing:0.12em;font-weight:700;text-transform:uppercase;color:#64748b;">Your details</p>
<table cellspacing="0" style="max-width:560px;">${h("Name", data.name)}${h("Phone", data.phone)}${h("Postcode", data.postcode)}${h("Service", data.service)}</table>
<p style="margin:20px 0 6px;font-weight:600;color:#0f172a;">What you told us</p>
<p style="margin:0;white-space:pre-wrap;color:#334155;">${
    data.details ? escapeHtml(data.details) : "—"
  }</p>
<p style="margin:24px 0 0;padding-top:16px;border-top:1px solid #e2e8f0;font-size:14px;color:#64748b;">Dripline Plumbers &middot; Harrow, Brent &amp; North West London<br/>
Need us sooner? <a href="tel:+447403767600" style="color:#0f172a;font-weight:600;">07403 767600</a></p>
</body></html>`;
...
export async function sendBookingEmail(env, data) {
  ...
  if (!hasKey || !hasFrom) {
    ...
    if (!hasFrom) missing.push("BOOKING_FROM_EMAIL");
    return {
      ok: false,
      status: 503,
  ...
  const customer = buildCustomerConfirmationMessage(data);
  const fromLine = `Dripline Plumbers <${from}>`;
  ...
  const teamJson = await resTeam.json().catch(() => ({}));
  if (!resTeam.ok) {
    return {
      ok: false,
      status: 502,
  ...
  const customerJson = await resCustomer.json().catch(() => ({}));
  if (!resCustomer.ok) {
    return {
      ok: true,
      teamId: teamJson.id,
      customerEmailSent: false,
      customerEmailError: customerJson,
    };
  }

  return { ok: true, teamId: teamJson.id, customerId: customerJson.id, customerEmailSent: true };
}
