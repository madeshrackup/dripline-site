import { buildCustomerBookingEmailHtml } from "./build-customer-booking-email-html.mjs";

/**
 * Resend: https://resend.com/docs/api-reference/emails/send-email
 * Set RESEND_API_KEY, BOOKING_FROM_EMAIL (domain verified in Resend; optional: onboarding@resend.dev for dev).
 * Optional: BOOKING_EMAIL_LOGO_URL — absolute https to wordmark; defaults to site /nav-wordmark.png on production domain.
...
export function buildCustomerConfirmationMessage(data) {
  ...
  const lines = [
    ...
    "",
    summaryLine,
    "Thanks for getting in touch. Here is a full copy of what you sent. We will confirm with you by phone or email as soon as we can.",
    "",
    "--------",
    `Name:     ${data.name}`,
    `Phone:    ${data.phone}`,
  ...
  const html = buildCustomerBookingEmailHtml(data, {});
...
export async function sendBookingEmail(env, data) {
  ...
  // Resend: only the customer is in "to" — if someone else (e.g. a Gmail) also gets a copy,
  // that is from the Resend account, domain forwarding, or a mail rule, not this payload.
